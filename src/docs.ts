...
| File | Contents |
|---|---|
| \`te-araroa.gpx\` | ${s.tracks} tracks (main route, transport connectors, ${s.bypasses} bypasses), ${n(s.waypoints)} waypoints typed \`hut\`/\`campsite\`/\`town\`/\`resupply\`/\`food\`/\`accommodation\`/\`caravan-park\`. The stable filename - link to this one |
| \`te-araroa-${meta.season}.gpx\` | the same bytes under this release's name |
| \`resupply-plan.csv\` | ${n(s.planRows)} sites in trail order: km, section, trail elevation, leg distances, leg ascent/descent, bunks, water, booking, phone, address, hours, DOC link |
| \`sections.csv\` | ${s.sections} official sections with km ranges |
| \`datasheet.csv\`, \`datasheet-resupply.csv\` | the same route through \`gpx-tools\`' \`processGpxTravelPlan\` |
| \`no-camping-areas.geojson\` | ${n(s.noCampingAreas)} restricted-camping polygons |
| \`te-araroa.meta.json\` | every GIS attribute per site, plus sections, connectors, route gaps, the numbers this README quotes, and source checksums |
`,
...
function projectPage(meta: Meta): Record<string, string> {
  ...
  const files: Array<[string, string]> = [
    [
      "te-araroa.gpx",
      `${s.tracks} tracks (main route, ferry connectors, ${s.bypasses} bypasses) and ${n(s.waypoints)} typed waypoints. Elevation throughout.`,
    ],
    [
      "resupply-plan.csv",
      `${n(s.planRows)} sites in trail order: km, section, elevation, leg distances and ascent, bunks, water, booking, phone, hours.`,
    ],
    ["sections.csv", `The ${s.sections} official sections with their km ranges.`],
    [
      "no-camping-areas.geojson",
      `${n(s.noCampingAreas)} restricted-camping polygons.`,
    ],
    [
      "te-araroa.meta.json",
      "Every GIS attribute per site, plus sections, ferry connectors, route gaps, and the SHA-256 of the source files this was built from.",
    ],
  ];
...
`,
  ...
  in orange. The published GPX has the full ${n(s.routePoints)}-point geometry.
`,
